...
- **Live personas** — `boot_persona`, `list_personas` (gated behind `TESTEADOR_MCP_ENABLE_LIVE=1`). Boot a running **dev** app already seeded into a persona (an `Actor`+`Fixture`) by invoking its `ext.stabilitas.*` service extensions over the VM service, then explore it by hand or via `flutter_driver`. `boot_persona` needs the app's **ws:// VM-service URI** (not the DTD URI); omit `persona` to exit fixture mode. A companion **DevTools extension** ([`extension/devtools_app/`](extension/devtools_app/)) offers the same as a VS Code panel that lists the personas and seeds the one you pick.
