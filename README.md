...
- **Live (dev_mate federation)** — gated behind `TESTEADOR_MCP_ENABLE_LIVE=1`; every tool needs the app's **ws:// VM-service URI** (not the DTD URI). The generic pair `federated_discover`/`federated_invoke` (from [`dev_mate_mcp`](https://github.com/Rodsevich/dev_mate)) discovers the running app's whole dev_mate catalog (personas, questline signals, forceable clock, appearance) and drives ANY of it — new app domains appear with zero MCP changes. Historical sugar on top: `boot_persona`, `list_personas` (omit `persona` to exit fixture mode) and `configure_questline` (its clock actions hit `ext.dev_mate.clock.*`; liturgical-hour labels resolve from the app's catalog presets). The old companion DevTools extension moved to dev_mate as the generic `dev_mate` DevTools tab.
